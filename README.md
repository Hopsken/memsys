...
To connect an agent, add `https://<hostname>/mcp` as an MCP server and sign in when it asks. Agents that cannot sign in use an API key from **Settings → MCP** instead (see [Sign-in and MCP access](#sign-in-and-mcp-access)).
...
- **D1** (`DB`) is the Worker's database: users, sessions, sign-in codes, rate-limit counters, API keys, and OAuth clients, consents, and signing keys. Schema in `worker/db/schema/`, one file per area (Better Auth's tables in `auth.ts`), migrations in `migrations/`, applied by `wrangler d1 migrations`.
...
## Sign-in and MCP access

[Better Auth](https://www.better-auth.com/) runs in the Worker under `/api/auth` and reads D1 directly. There are three credentials, and each reaches only its own side:

| Credential | Held by | Reaches |
| --- | --- | --- |
| Session cookie, from an emailed one-time code | The browser | The web UI and `/api/*`: memory, plugin settings, API key and app management |
| OAuth access token, from signing in through an agent | An agent | `/mcp` only, limited to the scopes the user allowed |
| API key (`memsys_…`), a Better Auth API key | An agent | `/mcp` only, every tool |

Neither agent credential stands in for a session, so an agent cannot change plugin settings, create API keys, or manage apps, and a session cookie does not open `/mcp`.

**OAuth.** Better Auth's MCP plugin makes the Worker an OAuth 2.1 authorization server for `/mcp`. A request without a token gets a 401 whose `WWW-Authenticate` header points to `/.well-known/oauth-protected-resource/mcp`; from there the agent finds the authorization server, registers itself (a Client ID Metadata Document or dynamic client registration), and sends the user to `/login` and then `/consent`. Two scopes decide which tools the agent sees: `memory:read` for `recall` and read-only plugin tools, `memory:write` for `remember`, `revise`, `forget`, and the other plugin tools. Access tokens are JWTs for the `/mcp` audience that last an hour; memsys adds `offline_access` to every authorization request, since apps such as ChatGPT ask only for the scopes `/mcp` lists, so any agent registered for refresh tokens can refresh them. `/mcp` also checks on every call that the consent still exists, so disconnecting an app under **Settings → MCP** takes effect at once and ends its refresh tokens.

**API keys** are for agents that cannot sign in. They are stored hashed; the web UI shows each one once, when it is created. Revoking a key takes effect on the next request.
...
Connect an agent with the URL alone and sign in when it asks, for example in Claude Code:

```sh
claude mcp add --transport http memsys https://<hostname>/mcp
```

Or with an API key as a bearer header:
...
Tests use real email sign-in, sessions, API keys, OAuth sign-in, D1, SQLite objects, object eviction, and MCP protocol requests. Only the Resend request is mocked; the test reads the code from it. No Cloudflare account is needed.
