...
- Association compares pure English-letter anchors with Porter2 stemming: `#program` and `#programming` link fragments. Anchors with numbers, non-English characters, `_`, `-`, or `/` still require exact lowercase matches. Stemming applies only to association, not cue matching or stored text. Returned `anchors` and `sharedAnchors` keep each fragment's lowercase tag spelling, not its stem.
